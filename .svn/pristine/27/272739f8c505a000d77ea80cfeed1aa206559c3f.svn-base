...
@interface MeViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITableViewDelegate,UITableViewDataSource,AsyncSocketDelegate,NSStreamDelegate>{
    UIAlertView *alert;
...
- (IBAction)uploadBtnClick:(id)sender {
    ...
    NSString *  sstr= [NSString stringWithFormat:@"%ld",self.dataArray.count];
        NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
    [defaults setValue:sstr forKey:@"count"];
    ...
    NSDictionary * params = @{@"userId":userId,@"operMBPS":@"阶段性反馈",@"time":currentDateStr,@"feedBackRemark":str,@"stepId":@"",@"woId":self.woId};
    ...
    [self.delegate feedBackValues:self.str andTimeString:self.timeStr];
...
-(void)jsonAttachDescripemainType:(NSString *)mainType mainRecId:(NSString *)mainRecId fileTitle:(NSString *)fileTitle attachType:(NSString *)attachType tag:(long)tag{
    ...
    if ( [[defaults objectForKey:@"count"] intValue] > 0) {
        
        AttachModel *attachModel = (AttachModel *)self.dataArray[(tag-1)/2];
        NSString *serveName;
        for (AttachGroupModels *model in modelArray) {
            if ([model.serveId intValue] == tag && model.groupId == _attachGroupId) {
        ...
        NSString *fileType = attachModel.fileType;
        fileType = @"0";
        NSString *fileName = attachModel.imgName;
        NSString *attachSpec = attachModel.imgDetails;

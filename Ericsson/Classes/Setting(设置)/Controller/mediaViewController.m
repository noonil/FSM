...
- (IBAction)uploadBtnClick:(id)sender {
    ...
    BOOL wifiOpen = [[defaults objectForKey:@"choose"]isEqualToString:@"Y"];
       ShareManager *shareManager = [ShareManager sharedManager];
    if (isWifi || !wifiOpen) {
        if (self.dataArray.count > 0 || shareManager.titleText) {
            [MBProgressHUD showMessage:@"正在反馈"];
            isConnected = [self connectServer:ASYNCHOST_IP port:ASYNCHOST_PORT];
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag
{
    ...
    if ((tag % 2) == 1) {
        ...
        clientSocket = nil;
        clientSocket.delegate = nil;
        if([self connectServer:ASYNCHOST_IP port:ASYNCHOST_PORT]){
